"""
Combined word + char TF-IDF features with LinearSVC.

Word-level:
    - analyzer='word', ngram_range=(1,2), max_features=12000, min_df=1, sublinear_tf=True
Char-level:
    - analyzer='char', ngram_range=(3,5), max_features=20000, min_df=1, sublinear_tf=True

To keep dimensionality manageable, the char TF-IDF block is reduced via TruncatedSVD (200 comps).
Structured features (20 dims) are standardized and concatenated with both text blocks.
"""
import numpy as np
import pandas as pd
import re
...
CHAR_SVD_COMPONENTS = int(os.environ.get("CHAR_SVD_COMPONENTS", "200"))
...
def build_features(df_train, df_test):
    ...
    X_train = hstack([X_struct_train, X_word_train, X_char_train_svd], format='csr')
    X_test = hstack([X_struct_test, X_word_test, X_char_test_svd], format='csr')

    return X_train, df_train['label'].values, X_test, df_test['label'].values
...
def main():
    ...
    best_score = -1
    best_C = None
    print("\n=== Word+Char LinearSVC Configurations ===")
    c_values = os.environ.get("LINEAR_SVC_C_VALUES")
    if c_values:
        c_values = [float(x) for x in c_values.split(",")]
    else:
        c_values = LINEAR_SVC_C_VALUES

    for C in c_values:
        clf = LinearSVC(C=C, max_iter=6000)
    ...
    final_clf = LinearSVC(C=best_C, max_iter=6000)
    ...
    print("\n=== Combined Word+Char Performance ===")
