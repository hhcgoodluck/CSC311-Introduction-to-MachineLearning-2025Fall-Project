"""
TF-IDF (1,2)-gram + structured features with LinearSVC / LogisticRegression.

Goals:
- Build a strong linear baseline optimized for sparse text + structured data.
- Student-wise 80/20 split to avoid leakage.
- Hyperparameter tuning via StratifiedKFold using accuracy (primary metric).
- Evaluate both LinearSVC and LogisticRegression; pick best configuration.
"""
import numpy as np
import pandas as pd
import re
...
def build_features(
    ...
    X_struct_train_scaled = scaler.fit_transform(X_struct_train_dense)
    X_struct_test_scaled = scaler.transform(X_struct_test_dense)

    X_struct_train = csr_matrix(X_struct_train_scaled)
    X_struct_test = csr_matrix(X_struct_test_scaled)
...
def main():
    ...
    X_train, y_train, X_test, y_test, vectorizer = build_features(
        df_train, df_test, tfidf_max_features=TFIDF_MAX_FEATURES, stop_words=TFIDF_STOP_WORDS
    )
    print(f"Feature shapes -> X_train: {X_train.shape}, X_test: {X_test.shape}")

    best_score = -1
    best_config = None
    print("\n=== LinearSVC Configurations ===")
    ...
    print(f"\nBest CV configuration: LinearSVC(C={best_config}), CV accuracy={best_score:.3f}")

    final_clf = LinearSVC(C=best_config, max_iter=5000)
